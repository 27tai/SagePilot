...
function WebhookTriggerNode({ data, selected }: NodeProps<RFNodeData>) {
  return (
    <div
    ...
    >
      {/* Header */}
      <div className="flex items-center gap-2 rounded-t-md bg-indigo-500 px-3 py-1.5">
        <span className="text-xs font-bold uppercase tracking-wide text-white">Webhook</span>
      </div>

      {/* Body */}
      <div className="px-3 py-2">
        <p className="text-sm font-medium text-gray-800">{data.label}</p>
        <p className="mt-0.5 text-xs text-gray-400">POST /api/webhooks/…</p>
      </div>
